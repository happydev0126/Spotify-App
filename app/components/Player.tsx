...
export default function Player({ token }: { token: string }) {
  ...
  const floatingPlayer =
    !fullScreen && isMobile && `fixed bottom-0 left-0 bg-background p-2`;

  if (!fullScreen)
    return (
      <div
        onClick={handleFullScreen}
        className={`flex h-16 w-full items-center justify-between col-span-full ${floatingPlayer}`}
      >
          ...
          {!isMobile && (
            <div className="w-full">
              <div className="flex flex-row items-center gap-1 text-sm text-zinc-400">
                <span>
                  {current_track &&
                  trackPositionInMs > current_track.duration_ms
