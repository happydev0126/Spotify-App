...
export default function PauseTrackButton({ token, variant }: { token: string, variant?: Variant }) {
  ...
  return (
    <button className={variant === 'green' ? 'flex items-center justify-center w-12 h-12 absolute bg-green z-50 bottom-2 right-2 rounded-full text-black shadow' : ''} onClick={e => handlePauseTrack(e)}>
